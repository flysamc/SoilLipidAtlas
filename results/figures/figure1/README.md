...
A Python generator builds the SVG from the analysis release, so every
printed number is live. Figure 1 is a workflow schematic, not a data plot,
which is why it is drawn in SVG rather than R.

## Design

Figure 1 carries the study design and its logic, not results. Panel d's
composition plot is an explicitly schematic sketch labelled "see Fig. 5".
The organism count is omitted (species count is not derived from the
metadata; see Table S1), as are per-mode sample counts (Table S1).

Six design-scale numbers remain: 19 collection phyla (16 in analysis),
6 batches, 273,248 POS / 122,571 NEG consensus features, 11,371 biomarkers
across 16 phyla, 736 verified soil fingerprint features.

## Files

| File | Purpose |
|---|---|
| `build_figure1_concept_v1.py` | Generator — builds the SVG from release CSVs and run summaries |
...
| `LEGEND.md` | The figure legend of record |
...
| `figure3/substrate/substrate_summary.json` | core sample counts (internal cross-check; not printed) |

To rasterise the preview from the SVG:
...
- Group colour palette: Bacteria `#7B52AB`, Archaea `#1B9E8F`, Fungi
  ...
  (locked display labels; some CSV keys remain Plantae/Protozoa).
