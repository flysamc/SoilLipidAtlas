...
Per-phylum SIMPER over the strict-16 atlas. `reproduction_validation.csv`
records the internal reproduce-first gate (recomputed contributions match the
release exactly). The full per-feature contribution table
(`simper_atlas.csv`, ~7 MB) and K-vs-recovery curves stay in the data
registry.

## MS2LDA substructure motifs (`scripts/run_ms2lda_deterministic.py`)

Deterministic MS2LDA (bit-for-bit reproducible across reruns; fixed seeds,
sorted inputs) on the atlas MS2 spectra, then motif–phylum enrichment
(producer `../04_annotation/scripts/strict_ms2lda_phylum_enrichment.py`):
**134 POS / 103 NEG significant motif–phylum pairs**. The enrichment table
feeds Supplementary Fig. 3 (results section).

## Cross-method validation (`cross_method/`, producer `scripts/suppfig8_cross_method_rerun.py`)

Four fingerprint-construction methods (SIMPER, SCBD, CAP, L1) compared on the
same substrate. SIMPER/SCBD are exact re-runs; CAP/L1 are declared
reimplementations. Panel data (Supplementary Fig. 8, Tables S11–S14):
...
- `panel_b_loo_accuracy.csv` — leave-one-out phylum assignment by method × K
  (L1 best ≈74%; CAP weakest) → Table S13.
...
`RUN_SUMMARY.json` carries substrates, seeds, and gate results.
